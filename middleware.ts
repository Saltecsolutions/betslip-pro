...
export async function middleware(request:NextRequest){
 ...
 const protectedPath=['/dashboard','/purchases','/tipster/','/admin','/account','/notifications'].some(p=>path.startsWith(p))||path==='/tipster';
 let target:string|null=null;
 if(protectedPath&&!user)target='/login?next='+encodeURIComponent(path+request.nextUrl.search);
 if(user&&protectedPath&&path!=='/account/privacy'){
...
export const config={matcher:['/dashboard/:path*','/purchases/:path*','/tipster/:path*','/admin/:path*','/account/:path*','/notifications/:path*']};
