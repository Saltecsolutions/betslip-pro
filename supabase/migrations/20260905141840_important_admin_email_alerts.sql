...
begin
 ...
 update admin_alerts.settings set enabled=p_enabled;
...
begin
 ...
 update admin_alerts.settings set last_worker_at=now(),worker_status=case when not enabled then 'paused' when p_configured then 'ready' else 'missing_resend_key' end;
